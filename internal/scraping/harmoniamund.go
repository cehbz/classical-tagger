...
func (e *HarmoniaMundiExtractor) Extract(url string) (*ExtractionResult, error) {
	...
	data := &AlbumData{
		Title:        MissingTitle, // Sentinel value
		OriginalYear: MissingYear,  // Will fail validation
	}

	result := NewExtractionResult(data).
		WithError(NewExtractionError("title", "HTML parsing not yet implemented", true)).
		WithError(NewExtractionError("year", "HTML parsing not yet implemented", true))

	return result, nil // Note: returns result even with errors
}

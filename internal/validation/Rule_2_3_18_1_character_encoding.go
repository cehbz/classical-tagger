...
func (r *Rules) CharacterEncoding(actual, reference *domain.Album) RuleResult {
	...
	if hasEncodingIssues(actual.Title()) {
		issues = append(issues, domain.NewIssue(
			...
			0,
			meta.id,
			fmt.Sprintf("Album title has character encoding issues: '%s'", actual.Title()),
		))
	}
	...
	for _, track := range actual.Tracks() {
		...
		if hasEncodingIssues(track.Title()) {
			issues = append(issues, domain.NewIssue(
				...
				track.Track(),
				meta.id,
				fmt.Sprintf("Track %s: Title has character encoding issues: '%s'",
		...
		for _, artist := range track.Artists() {
			if hasEncodingIssues(artist.Name()) {
				issues = append(issues, domain.NewIssue(
					...
					track.Track(),
					meta.id,
					fmt.Sprintf("Track %s: Artist name has character encoding issues: '%s'",
		...
		if hasEncodingIssues(track.Name()) {
			issues = append(issues, domain.NewIssue(
				...
				track.Track(),
				meta.id,
				fmt.Sprintf("Track %s: Filename has character encoding issues: '%s'",

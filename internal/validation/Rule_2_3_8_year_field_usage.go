	...
	if year != 0 {
		...
		if year < 1900 {
			issues = append(issues, domain.NewIssue(
				...
				0,
				meta.id,
				fmt.Sprintf("Year %d seems too early for a recording (check if correct)", year),
			))
		}
		...
		if year > 2026 {
			issues = append(issues, domain.NewIssue(
				...
				0,
				meta.id,
				fmt.Sprintf("Year %d is in the future (use recording/release year)", year),
			))
		}
	}
	...
	if reference != nil && reference.OriginalYear() != 0 {
		...
		if year != 0 && year != refYear {
			...
			issues = append(issues, domain.NewIssue(
				...
				0,
				meta.id,
				fmt.Sprintf("Year %d differs from reference %d (difference: %d years)",
					year, refYear, diff),
			))
		}
	}
		...
		if editionYear != 0 && year != 0 {
			...
			if editionYear < year {
				issues = append(issues, domain.NewIssue(
					...
					0,
					meta.id,
					fmt.Sprintf("Edition year %d is earlier than album year %d (check if correct)",
						editionYear, year),
				))
			}
		}

...
func (r *Rules) AlbumArtistTag(actualTorrent, _ *domain.Torrent) RuleResult {
	meta := RuleMetadata{
		ID:     "2.3.7-present",
	...
	if len(actualTorrent.AlbumArtist) > 0 {
		albumArtistStr := domain.FormatArtists(actualTorrent.AlbumArtist)
		if !strings.EqualFold(strings.TrimSpace(albumArtistStr), "Various Artists") {
			...
			for _, aa := range actualTorrent.AlbumArtist {
	...
	if strings.Contains(strings.ToLower(actualTorrent.Title), "various artists") {
		issues = append(issues, domain.ValidationIssue{
			Level:   domain.LevelInfo,
			Track:   0,
			Rule:    meta.ID,
	...
	if totalTracks == 1 {
		...
		for _, a := range actualTorrent.Tracks()[0].Artists {

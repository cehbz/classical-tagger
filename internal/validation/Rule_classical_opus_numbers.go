		...
		for _, actualTrack := range actualTracks {
			...
			if refHasOpus && !actualHasOpus {
				...
				issues = append(issues, domain.NewIssue(
					...
					actualTrack.Track(),
					meta.id,
					fmt.Sprintf("Track %s: Consider adding opus/catalog number '%s' from reference",
			...
			if !hasOpusNumber(track.Title()) {
				...
				if needsCatalogNumber(composer) {
					issues = append(issues, domain.NewIssue(
						...
						track.Track(),
						meta.id,
						fmt.Sprintf("Track %s: Consider adding opus/catalog number for better identification",

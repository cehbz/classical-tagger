		...
		if containsWord(albumTitle, lastName) {
			...
			if !containsWord(albumTitle, fullName) && !isAcceptableAbbreviation(albumTitle, fullName) {
				issues = append(issues, domain.NewIssue(
					...
					0, // Album-level
					meta.id,
					fmt.Sprintf("Album title contains composer surname '%s' but not full name '%s' (full name recommended)",
						lastName, fullName),
				))
			}
		} else {
			...
			issues = append(issues, domain.NewIssue(
				...
				0,
				meta.id,
				fmt.Sprintf("Album title does not mention primary composer '%s' (recommended for classical albums)",
					fullName),
			))
		}
	}
